...
class StringPreHandler:
    ...
    def number_translator(cls, target):
        ...
        for m in match:
            ...
            num = 0
            if len(s) == 2:
                num += cls.word_to_number(s[0]) * 10000 + cls.word_to_number(s[1]) * 1000
        ...
        for m in match:
            ...
            num = 0
            if len(s) == 2:
                num += cls.word_to_number(s[0]) * 1000 + cls.word_to_number(s[1]) * 100
            target = pattern.sub(str(num), target, 1)
        ...
        for m in match:
            ...
            num = 0
            if len(s) == 2:
                num += cls.word_to_number(s[0]) * 100 + cls.word_to_number(s[1]) * 10
            target = pattern.sub(str(num), target, 1)
        ...
        for m in match:
            ...
            s = group.split(u"十")
            num = 0
            ten = cls.str_to_int(s[0])
            if ten == 0:
                ten = 1
            unit = cls.str_to_int(s[1])
            num = ten * 10 + unit
            target = pattern.sub(str(num), target, 1)
