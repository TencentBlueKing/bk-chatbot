...
class CommandSession(BaseSession):
    ...
    def get(self, key: str, *,
        ...
        if key in self.state:
            return self.state[key], self.ctx
...
async def handle_command(bot: Bot, ctx: Context_T) -> bool:
    ...
    if session:
        ...
        if session.is_valid:
            ...
            ctx['to_me'] = True
            if str(ctx['message']) not in session.bot.config.SESSION_RESERVED_WORD:
...
async def _real_run_command(session: CommandSession,
    ...
    try:
        ...
        if isinstance(e, _FinishException):
            ...
            # this is intended to be propagated to handle_message()
            raise e
...
def is_session_exist(ctx: Context_T) -> bool:
    return context_id(ctx) in _sessions


from opsbot.command.group import CommandGroup
