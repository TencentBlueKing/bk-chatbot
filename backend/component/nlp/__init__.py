...
STOP_WORDS_PATH = os.path.join(CUR_PATH, 'word_manage', 'stopwords.txt')
SIMILAR_WORD_PATH = os.path.join(CUR_PATH, 'word_manage', 'similar_word.json')
...
def sort_by_similar(sort, biz_data_list):
    ...
    for i in sort:
        if i[1] >= similar_percentage:
            temp = {'utterance': biz_data_list[i[0]]['utterance'],
